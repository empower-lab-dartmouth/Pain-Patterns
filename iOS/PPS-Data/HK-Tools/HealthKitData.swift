        ...
        healthStore.requestAuthorization(toShare: nil, read: allTypes) { (success, error) in
            ...
            for type in allTypes {
                let query = HKSampleQuery(sampleType: type, predicate: nil, limit: HKObjectQueryNoLimit, sortDescriptors: nil) { (query, samples, error) in
                    ...
                    let samples = actualSamples as? [HKQuantitySample]
//                    print(samples, "\n")
                }
...
func healthDataTypes() -> Set<HKSampleType> {
    var typesToUse: Set<HKSampleType> = [] // this is the set that contains all information you want to get
